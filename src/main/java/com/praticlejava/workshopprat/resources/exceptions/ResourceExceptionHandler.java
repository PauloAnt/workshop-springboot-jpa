package com.praticlejava.workshopprat.resources.exceptions;
...
public class ResourceExceptionHandler {
	...
	public ResponseEntity<StandardError> resourceNotFound(ResourceNotFoundException r, HttpServletRequest request){
		...
		StandardError err = new StandardError(Instant.now(), status.value(), error, r.getMessage(), request.getRequestURI());
		return ResponseEntity.status(status).body(err);
	}
	
	@ExceptionHandler(DatabaseException.class)
	public ResponseEntity<StandardError> database(DatabaseException r, HttpServletRequest request){
